...
class Grid:
    def __init__(self, ui=False):
        ...
        self.height = 25
        self.nodes = {
            (i, j): Node((i, j)) for i in range(self.height) for j in range(self.width)
        }
    ...
    def update(self, game, pygame):
        ...
        for i in range(self.width + 1):
            pygame.draw.line(
                game.screen,
                [100] * 3,
                (NODE_W * i + OFFSET[0], OFFSET[1]),
                (NODE_W * i + OFFSET[0], OFFSET[1] + NODE_W * self.height),
            )
        for i in range(self.height + 1):
            pygame.draw.line(
                game.screen,
                [100] * 3,
                (OFFSET[0], (NODE_W * i) + OFFSET[1]),
                (OFFSET[0] + NODE_W * self.width, (NODE_W * i) + OFFSET[1]),
            )
