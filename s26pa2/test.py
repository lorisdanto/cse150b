...
def print_test_result(result, item, score, sol_score):
    if result:
        print("PASSED: Correct {}.".format(item))
        return True
    else:
        print(f"FAILED: Incorrect {item}. Expected: {sol_score:.3f}, Got: {score:.3f}")
        return False
...
def test(board_file='test_states', sol_file='test_sols'):
    ...
    for i in range(len(state_lines)):
        ...
        res = print_test_result((score >= sol_score - TOL) and score <= (sol_score + TOL), "expected score", score, sol_score)
        if not res:
            print("\n""Stopping tests here until fixed.")
            break
...
def test_extension():
    ...
    for i in range(NUM_TESTS):
        ...
        while not game.game_over():
            ...
            direction = ai.compute_decision_extension()
